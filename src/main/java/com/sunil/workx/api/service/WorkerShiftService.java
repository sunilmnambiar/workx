...
public class WorkerShiftService {
	...
	public List<WorkerShiftDTO> getAllWorkerShifts() {
		return workerShiftRepository.findAll().stream()
				.map(workerShift -> buildWorkerShiftDTO(workerShift)).toList();
	}

	public WorkerShiftDTO getWorkerShift(Long id) {
		WorkerShift workerShift = workerShiftRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("WorkerShift with id: " + id));
	...
	public WorkerShiftDTO upsertWorkerShift(WorkerShiftDTO workerShiftDTO) {
		try {
			...
			String errMsg = "Invalid worker '" + workerShiftDTO.getWorkerId()
					+ "' and/or shift '" + workerShiftDTO.getShiftId() + "'";
			log.error(errMsg, e);
	...
	public void deleteWorkerShift(Long id) {
		workerShiftRepository.findById(id).orElseThrow(() -> new BadRequestException("WorkerShift with id: " + id + " not found"));
	...
	private WorkerShiftDTO buildWorkerShiftDTO(WorkerShift workerShift) {
		if(workerShift != null) {
			return WorkerShiftDTO.builder()
					...
					.workerId(workerShift.getWorker().getId())
					.shiftId(workerShift.getShift().getId())
					.build();
		} else {
			return null;
		}
	...
	private WorkerShift buildWorkerShift(WorkerShiftDTO workerShiftDTO) {
		if(workerShiftDTO != null) {
			return WorkerShift.builder()
					.id(workerShiftDTO.getId())
					.date(workerShiftDTO.getDate())
					.worker(workerRepository.findById(workerShiftDTO.getWorkerId()).orElseThrow(() -> new BadRequestException("Worker with id: " + workerShiftDTO.getWorkerId() + " not found")))
					.shift(shiftRepository.findById(workerShiftDTO.getShiftId()).orElseThrow(() -> new BadRequestException("Shift with id: " + workerShiftDTO.getShiftId() + " not found")))
					.build();
		} else {
			return null;
		}
	...
	public WorkerShiftService(WorkerShiftRepository workerShiftRepository, 
			WorkerRepository workerRepository, ShiftRepository shiftRepository) {
